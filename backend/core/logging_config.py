...
def get_logging_config() -> dict[str, Any]:
    ...
    env_mode = os.getenv("ENVIRONMENT", "LOCAL").upper()
    log_level = os.getenv("LOG_LEVEL", "INFO").upper()

    sentry_logging = LoggingIntegration(
        level=None,  # Do NOT capture all logs
        event_level=None,  # We'll forward logs via structlog instead
    )
    ...
    if env_mode in ("LOCAL", "DEVELOPMENT"):
        shared_processors.append(remove_sentry_id)
